...
var base64Url = index$10.fromBase64;
var Buffer$4 = index$12.Buffer;
...
function createHmacSigner(bits) {
  return function sign(thing, secret) {
    ...
    return index$10.fromBase64(sig);
  }
}
...
function createHmacVerifier(bits) {
  return function verify(thing, signature, secret) {
    ...
    return index$16(Buffer$2.from(signature), Buffer$2.from(computedSig));
  }
}
...
function createKeyVerifier(bits) {
  return function verify(thing, signature, publicKey) {
    ...
    thing = normalizeInput(thing);
    signature = index$10.toBase64(signature);
...
var index$14 = function jwa(algorithm) {
  var signerFactories = {
    hs: createHmacSigner,
    rs: createKeySigner,
...
function jwsSecuredInput(header, payload, encoding) {
  ...
  var encodedHeader = index$10(tostring(header), 'binary');
  var encodedPayload = index$10(tostring(payload), encoding);
...
function headerFromJWS(jwsSig) {
  ...
  return safeJsonParse(index$10.decode(encodedHeader, 'binary'));
}
...
function payloadFromJWS(jwsSig, encoding) {
  ...
  return index$10.decode(payload, encoding);
}
...
var decode = function (jwt, options) {
  options = options || {};
  var decoded = index$8.decode(jwt, options);
...
var index$18 = function (val, options) {
  options = options || {};
  var type = typeof val;
  if (type === 'string' && val.length > 0) {
    return parse$1(val)
...
var verify$1 = function (jwtString, secretOrPublicKey, options, callback) {
  ...
  var decodedToken;
  try {
    decodedToken = index$8.decode(jwtString);
  } catch(err) {
    return done(err);
  }
  ...
  try {
    valid = index$8.verify(jwtString, header.alg, secretOrPublicKey);
  } catch (e) {
    return done(e);
  }
  ...
  if (options.maxAge) {
    var maxAge = index$18(options.maxAge);
...
var timespan = function (time, iat) {
  ...
  if (typeof time === 'string') {
    var milliseconds = index$18(time);
...
var sign_options_schema = {
  expiresIn: function(value) { return index$26(value) || index$32(value); },
  notBefore: function(value) { return index$26(value) || index$32(value); },
  audience: function(value) { return index$32(value) || index$22(value); },
  algorithm: index$20.bind(null, ['RS256', 'RS384', 'RS512', 'ES256', 'ES384', 'ES512', 'HS256', 'HS384', 'HS512', 'none']),
  header: index$30,
  encoding: index$32,
  issuer: index$32,
  subject: index$32,
  jwtid: index$32,
  noTimestamp: index$24,
  keyid: index$32
};
...
var sign$1 = function (payload, secretOrPrivateKey, options, callback) {
  ...
  if (typeof callback === 'function') {
    callback = callback && index$34(callback);
    ...
    return index$8.sign({header: header, payload: payload, secret: secretOrPrivateKey, encoding: encoding});
  }
};
...
exports.signServiceSecret = () => index$6.sign({ baseUrl: baseUrl.baseUrl }, secret, { algorithm: 'HS256', expiresIn: 30 });
exports.verifyServiceSecret = (opaqueObject) => {
    const { baseUrl: foundBaseUrl } = index$6.verify(opaqueObject, secret, { algorithms: ['HS256'], clockTolerance: 30 });
...
const { get, post } = fetch.default('topup');
exports.getTopupAccount = (key, id, cacheBypass = false) => get(1, key, `/${id}`, cacheBypass);
exports.createTopup = (key, request) => post(1, key, '/', request);
exports.recordTransaction = (key, { transaction, balance }) => post(1, key, '/transaction', Object.assign({}, transaction, { balance }));
exports.getCardDetails = (key, accountId) => get(1, key, `/${accountId}/cardDetails`);
});
...
const asyncHandler = (event) => __awaiter(commonjsGlobal, void 0, void 0, function* () {
    const key$$1 = key.createServiceKey({ service: 'transaction-topup' });
    for (const transaction of stream$1.subscribeTransactionsAndBalances(event)) {
        yield index$2.recordTransaction(key$$1, transaction);
    }
