...
const createKey = ({ service, correlationKey = index.v4(), tags = {} }) => {
    return {
...
const isTopupAccount = (account) => account.test != null && account.userId != null;
exports.subscribeTopups = function* (event) {
...
var assertString_1$1 = createCommonjsModule(function (module, exports) {
'use strict';
...
var isUUID_1$1 = createCommonjsModule(function (module, exports) {
'use strict';
...
var _assertString2 = _interopRequireDefault(assertString_1$1);
...
var util$1 = ( util && util__default ) || util;
...
LRUCache.prototype.inspect = function (n, opts) {
  ...
  if (maxAge) {
    ...
    str += '\n  maxAge: ' + util$1.inspect(maxAge, opts);
    extras = true;
  }
  ...
  if (lc && lc !== naiveLength) {
    ...
    str += '\n  length: ' + util$1.inspect(priv(this, 'length'), opts);
    extras = true;
  }
  ...
  priv(this, 'lruList').forEach(function (item) {
    ...
    val = util$1.inspect(val, opts).split('\n').join('\n  ');
    str += key + ' => ' + val;
  });
...
var require$$0$2 = ( buffer && buffer__default ) || buffer;
...
var Stream = ( stream && stream__default ) || stream;

/*global module, process*/
var Buffer$1 = index$14.Buffer;
...
util$1.inherits(DataStream, Stream);
...
var Buffer$3 = require$$0$2.Buffer; // browserify
var SlowBuffer = require$$0$2.SlowBuffer;

var index$18 = bufferEq;
...
var base64Url = index$12.fromBase64;
var Buffer$4 = index$14.Buffer;
...
function typeError(template) {
  ...
  var errMsg = util$1.format.bind(util$1, template).apply(null, args);
...
function createHmacSigner(bits) {
  return function sign(thing, secret) {
    ...
    return index$12.fromBase64(sig);
  }
}
...
function createHmacVerifier(bits) {
  return function verify(thing, signature, secret) {
    ...
    return index$18(Buffer$2.from(signature), Buffer$2.from(computedSig));
  }
}
...
function createKeyVerifier(bits) {
  return function verify(thing, signature, publicKey) {
    ...
    thing = normalizeInput(thing);
    signature = index$12.toBase64(signature);
...
var index$16 = function jwa(algorithm) {
  var signerFactories = {
    hs: createHmacSigner,
    rs: createKeySigner,
...
function jwsSecuredInput(header, payload, encoding) {
  ...
  var encodedHeader = index$12(tostring(header), 'binary');
  var encodedPayload = index$12(tostring(payload), encoding);
...
util$1.inherits(SignStream, Stream);
...
function headerFromJWS(jwsSig) {
  ...
  return safeJsonParse(index$12.decode(encodedHeader, 'binary'));
}
...
function payloadFromJWS(jwsSig, encoding) {
  ...
  return index$12.decode(payload, encoding);
}
...
var decode = function (jwt, options) {
  options = options || {};
  var decoded = index$10.decode(jwt, options);
...
var index$20 = function (val, options) {
  options = options || {};
  var type = typeof val;
  if (type === 'string' && val.length > 0) {
    return parse$2(val)
...
var verify$1 = function (jwtString, secretOrPublicKey, options, callback) {
  ...
  var decodedToken;
  try {
    decodedToken = index$10.decode(jwtString);
  } catch(err) {
    return done(err);
  }
  ...
  try {
    valid = index$10.verify(jwtString, header.alg, secretOrPublicKey);
  } catch (e) {
    return done(e);
  }
  ...
  if (options.maxAge) {
    var maxAge = index$20(options.maxAge);
...
var timespan = function (time, iat) {
  ...
  if (typeof time === 'string') {
    var milliseconds = index$20(time);
...
function isString$1(value) {
  return typeof value == 'string' ||
    (!isArray$2(value) && isObjectLike$5(value) && objectToString$5.call(value) == stringTag$1);
}

var index$34 = isString$1;
...
var sign_options_schema = {
  expiresIn: function(value) { return index$28(value) || index$34(value); },
  notBefore: function(value) { return index$28(value) || index$34(value); },
  audience: function(value) { return index$34(value) || index$24(value); },
  algorithm: index$22.bind(null, ['RS256', 'RS384', 'RS512', 'ES256', 'ES384', 'ES512', 'HS256', 'HS384', 'HS512', 'none']),
  header: index$32,
  encoding: index$34,
  issuer: index$34,
  subject: index$34,
  jwtid: index$34,
  noTimestamp: index$26,
  keyid: index$34
};
...
var sign$1 = function (payload, secretOrPrivateKey, options, callback) {
  ...
  if (typeof callback === 'function') {
    callback = callback && index$36(callback);
    ...
    return index$10.sign({header: header, payload: payload, secret: secretOrPrivateKey, encoding: encoding});
  }
};
...
exports.signServiceSecret = () => index$8.sign({ baseUrl: baseUrl.baseUrl }, secret, { algorithm: 'HS256', expiresIn: 30 });
exports.verifyServiceSecret = (opaqueObject) => {
    const { baseUrl: foundBaseUrl } = index$8.verify(opaqueObject, secret, { algorithms: ['HS256'], clockTolerance: 30 });
...
var url_1 = ( url && url__default ) || url;
...
const getFunctionPrefix = (baseUrl$$1) => {
    const { hostname } = url_1.parse(baseUrl$$1);
    if (hostname == null) {
...
exports.balanceLimit = 1000; // £10
exports.AUTO_REFUND_PERIOD = index$4('24h');
exports.transactionTypes = ['topup', 'purchase', 'refund', 'debit', 'credit'];
const isSHA256 = (hash) => /^[a-f0-9]{64}$/.test(hash);
exports.extractFieldsFromTransactionId = (id) => {
...
exports.assertValidTransactionId = (id) => {
    const { accountId, transactionId, rest } = exports.extractFieldsFromTransactionId(id);
    if (rest.length) {
        throw new Error(`Transaction id isn't <accountId>:<transactionHash> (${id})`);
    }
    if (!isUUID_1$1(accountId)) {
...
const assertValidLegacyId = assert.createAssertValidUuid('legacyId');
const isValidTransactionType = (type) => exports.transactionTypes.some(x => x === type);
exports.assertValidTransaction = (_a) => {
    var { type, amount, data, id, other, next, legacyId, timestamp } = _a, rest = __rest(_a, ["type", "amount", "data", "id", "other", "next", "legacyId", "timestamp"]);
    exports.assertValidTransactionId(id);
    ...
    if (other != null) {
        exports.assertValidTransactionId(other);
    }
    if (next != null) {
        exports.assertValidTransactionId(next);
    }
    if (legacyId != null) {
...
const { get, post } = fetch.default('transaction');
exports.createAccount = (key, accountId) => post(1, key, '/account', { accountId });
exports.getAccount = (key, accountId) => get(1, key, `/account/${accountId}`);
exports.createTransaction = (key, accountId, transaction) => post(1, key, `/account/${accountId}`, transaction);
exports.getTransaction = (key, transactionId) => get(1, key, `/tx/${transactionId}`);
exports.issueUserRequestedRefund = (key, transactionId, userId, reason) => post(1, key, `/tx/${transactionId}/refund/user`, { userId, reason });
exports.issueSupportRequestedRefund = (key, transactionId, reason, dateLimit) => post(1, key, `/tx/${transactionId}/refund/support`, { dateLimit, reason });
exports.recordTopup = (key, accountId, id, transaction) => post(1, key, `/account/${accountId}/topup/${id}`, transaction);
exports.assertBalanceWithinLimit = ({ key, accountId, amount }) => __awaiter(commonjsGlobal, void 0, void 0, function* () {
    const currentBalance = (yield exports.getAccount(key, accountId)).balance;
    ...
    if (currentBalance + amount > exports.balanceLimit) {
        throw new error$1.CodedError('MaxBalanceExceeded', `topping up would increase balance over the limit of £${exports.balanceLimit / 100}`);
    }
});
exports.TEST_DATA_EMPTY_ACCOUNT_ID = 'b423607f-64de-441f-ac39-12d50aaedbe9';
});
...
const asyncHandler = (event) => __awaiter(commonjsGlobal, void 0, void 0, function* () {
    const key$$1 = key.createServiceKey({ service: 'transaction-store' });
    for (const topupAccount of stream$1.subscribeTopups(event)) {
        if (topupAccount.status == null || topupAccount.status.status !== 'in-progress') {
            return;
        }
        yield index$2.recordTopup(key$$1, topupAccount.id, topupAccount.status.chargeId, {
            type: 'topup',
